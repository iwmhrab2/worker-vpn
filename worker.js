    ...
    if (sub) {
        ...
        if ((addresses.length + addressesapi.length + addressescsv.length) == 0) {
            ...
            addresses = addresses.concat(
                cfips.map(cidr => generateRandomIPFromCIDR(cidr) + ':' + randomPorts[Math.floor(Math.random() * randomPorts.length)] + '#ushift' + String(counter++).padStart(2, '0'))
            );
        }
    }
        ...
        const html = `
            ...
            <body>
                ...
                <div class="editor-container">
                    ...
                    ${cmad}
                    ` : `
                    <div style="background: rgba(255, 77, 77, 0.1); border: 1px solid #ff4d4d; border-radius: 12px; padding: 24px; margin-top: 20px; color: #fff;">
                        <h3 style="color: #ff4d4d; margin-bottom: 12px;">🚫 فضا ذخیره‌سازی KV متصل نیست | KV Not Bound</h3>
                        <p style="margin-bottom: 16px; line-height: 1.6; opacity: 0.9;">
                           برای استفاده از قابلیت ویرایش لیست گره‌ها، باید یک <b>KV Namespace</b> در پنل کلودفلر ایجاد کرده و آن را با نام <b>KV</b> به این ورکر متصل کنید.
                        </p>
                        <ul style="margin-bottom: 20px; opacity: 0.8; font-size: 0.9rem;">
                            <li>Settings -> Variables -> KV Namespace Bindings</li>
                            <li>Variable Name: <b>KV</b></li>
                        </ul>
                        <button class="back-btn" onclick="goBack()">بازگشت به داشبورد</button>
                    </div>
                    `}
                </div>
...
<head>
    ...
    <title id="pageTitle">ushift | Enterprise Access</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero h1 {
            ...
            font-weight: 700;
            margin-bottom: 12px;
            background: linear-gradient(135deg, #fff 0%, #7c4dff 100%);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
        }

        .hero p { color: var(--text-dim); font-size: 1.1rem; margin-bottom: 40px; }
        ...
        .stat-value { font-size: 2rem; font-weight: 600; margin-top: 12px; color: #fff; }
        ...
        @media (max-width: 480px) {
            ...
            .sub-link-card div { flex-direction: column; }
        }
        ...
        .modal.active { display: flex; animation: modalIn 0.3s ease-out; }
        ...
        @media (max-width: 480px) {
            .modal-content { padding: 32px 16px; border-radius: 20px; }
            .glass-card { padding: 24px 20px; }
        }

        @media (max-width: 480px) {
            .modal-content { padding: 32px 20px; border-radius: 24px; }
            #qrTarget { padding: 16px !important; border-radius: 16px !important; }
        }
        ...
        .footer { text-align: center; padding: 64px 0; color: var(--text-dim); font-size: 0.9rem; opacity: 0.5; }
        ...
        @media (max-width: 900px) {
            ...
            .sidebar { 
                ...
                background: rgba(5, 6, 15, 0.98) !important;
                backdrop-filter: blur(30px) !important;
                padding-top: 40px !important;
            }
            .sidebar.open { left: 0 !important; box-shadow: 20px 0 50px rgba(0,0,0,0.8); }
            .sidebar-overlay { 
                display: none;
                position: fixed;
                inset: 0;
...
<body>
    ...
    <div class="layout">
        <aside id="mainSidebar" class="sidebar">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div class="brand">🚀 ushift<span>.</span></div>
                <div class="nav-item" style="padding:10px; border-radius:10px; display:none;" id="closeSidebar" onclick="toggleMenu()">✕</div>
            </div>
            <nav>
                <ul class="nav-list">
                    <li class="nav-item active" onclick="showPage('dashboard', event)">🛰️ Dashboard</li>
                    <li class="nav-item" onclick="showPage('subs', event)">📦 Subscriptions</li>
                    <li class="nav-item" onclick="showPage('meta', event)">📊 System Meta</li>
                </ul>
            </nav>
        </aside>
        ...
        <main class="main">
            <div class="mobile-header">
                <div class="brand" style="font-size:1.4rem;">ushift<span>.</span></div>
                <div class="menu-btn" onclick="toggleMenu()">☰</div>
            </div>
            <div id="dashboard" class="page">
                <div class="hero">
                    <h1>Enterprise Connectivity</h1>
                    <p>High-performance edge management gateway.</p>
                </div>

                <div class="stat-grid">
                    <div class="glass-card stat-card">
                        <div class="stat-label">Security State</div>
                        <div class="stat-value" style="color:var(--accent);">Active</div>
                    </div>
                    <div class="glass-card stat-card">
                        <div class="stat-label">Main Gateway</div>
                        <div class="stat-value" id="mainHost">---</div>
                    </div>
                </div>
            </div>

            <div id="subs" class="page" style="display:none;">
                <h2 style="margin-bottom:32px;">Active Stream Formats</h2>
                <div id="subContainer"></div>
            </div>

            <div id="meta" class="page" style="display:none;">
                <div class="glass-card">
                    <h2 style="margin-bottom:24px;">Gateway Metadata</h2>
                    <div id="metaGrid" style="display:grid; gap:16px;"></div>
                </div>
            </div>

            <footer class="footer">
                <p id="uaText"></p>
                <p style="margin-top:12px;">ushift Matrix v5.0 • Scalable Edge Infrastructure</p>
            </footer>
        </main>
    </div>
    ...
    <div id="qrModal" class="modal">
        <div class="modal-content">
            <h2 style="margin-bottom:12px;">Sync Profile</h2>
            <p style="color:var(--text-dim); margin-bottom:32px;">Scan with your mobile client to handover.</p>
            <div id="qrTarget" style="background:#fff; padding:24px; border-radius:24px; display:inline-block; margin-bottom:32px;"></div>
            <div><button class="btn btn-outline" onclick="closeModal('qrModal')">Dismiss</button></div>
        </div>
    </div>

    <div id="toast">Matrix updated</div>
    ...
    <script>
        ...
        async function init() {
            try {
                const res = await fetch(window.location.pathname + '/config.json?token=${token}');
                if (!res.ok) throw new Error('Fetch failed');
                config = await res.json();
                render();
            } catch (e) {
                console.error('Initialization failed', e);
                document.getElementById('uaText').innerText = 'System Link Offline - Check Network';
            }
        }
        ...
        function render() {
            ...
            if (mainHostEl) mainHostEl.innerText = config.config.HOST || 'Unknown';
            
            const uaTextEl = document.getElementById('uaText');
            if (uaTextEl) uaTextEl.innerText = 'CLIENT IDENT: ' + (config.UA || 'Unknown');
            
            const p = config.config.KEY.PASSWORD;

            // Subs
            const cont = document.getElementById('subContainer');
            if (cont) {
                const streams = [
                    { n: 'Smart Adaptive Profile', s: '?sub' },
                    { n: 'Base64 Legacy Archive', s: '?b64' },
                    { n: 'Clash Matrix Config', s: '?clash' },
                    { n: 'SingBox Advanced', s: '?sb' }
                ];
                let h = '';
                for (const s of streams) {
                    const url = 'https://' + config.config.HOST + '/' + p + s.s;
                    h += '<div class="glass-card sub-link-card">';
                    h += '<h3 style="margin-bottom:8px;">' + s.n + '</h3>';
                    h += '<div class="link-text" onclick="copy(\\'' + url + '\\')">' + url + '</div>';
                    h += '<div style="display:flex; gap:12px;">';
                    h += '<button class="btn btn-outline" onclick="copy(\\'' + url + '\\')">Copy Link</button>';
                    h += '<button class="btn btn-outline" onclick="showQR(\\'' + url + '\\')">Sync QR</button>';
                    h += '</div></div>';
                }
                cont.innerHTML = h;
            }
            ...
            if (grid) {
                const items = [
                    { k: 'Origin Domain', v: config.config.HOST },
                    { k: 'Security Token', v: p },
                    { k: 'Traffic Backhaul', v: (config.proxyip && config.proxyip.GO2CF ? config.proxyip.GO2CF : '---').toUpperCase() },
                    { k: 'Matrix Engine', v: (config.sub && config.sub.SUB ? config.sub.SUB : 'Internal') }
                ];
                let gh = '';
                for (const i of items) {
                    gh += '<div style="display:flex; justify-content:space-between; align-items:center; padding:20px; border-radius:18px; background:rgba(255,255,255,0.02); border:1px solid var(--glass-border);">';
                    gh += '<span style="color:var(--text-dim); font-size:0.95rem;">' + i.k + '</span>';
                    gh += '<span style="font-family:var(--mono); color:#fff; font-size:0.95rem;">' + i.v + '</span>';
                    gh += '</div>';
                }
                grid.innerHTML = gh;
            }
        ...
        function copy(t) {
            ...
            if (ts) {
                ts.innerText = 'Profile Linked to Clipboard';
                ts.classList.add('active');
                setTimeout(() => ts.classList.remove('active'), 2500);
            }
        }
        ...
        function showQR(t) {
            ...
            if (target) {
                target.innerHTML = '';
                new QRCode(target, { text: t, width: 220, height: 220, colorDark: "#000" });
